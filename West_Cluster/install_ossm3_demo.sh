...
oc label namespace istio-system topology.istio.io/network=network2

echo "Installing  certificates"
oc get secret -n istio-system cacerts || oc create secret generic cacerts -n istio-system \
  --from-file=../certs/root-cert/west/ca-cert.pem \
  --from-file=../certs/root-cert/west/ca-key.pem \
  --from-file=../certs/root-cert/west/root-cert.pem \
  --from-file=../certs/root-cert/west/cert-chain.pem
